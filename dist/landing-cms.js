...
(function() {
  function getPageKey() {
    ...
    if (path.indexOf('sky-dive') !== -1) return 'skydive';
  ...
  function applyLandingCMS() {
    ...
    if (pageKey === 'skydive' && custom) {
      if (Array.isArray(custom.packages)) {
        custom.packages = custom.packages.filter(function(p) {
          return p.id !== 'skydive-desert' && (!p.name || p.name.indexOf('Desert') === -1);
        });
        custom.packages.forEach(function(p) {
          if (p.id === 'skydive-palm') {
            p.price = 2700;
          }
        });
      }
